...
func mapLibraryName(frame *rtda.Frame) {
	vars := frame.LocalVars()
	jSrcStr := vars.GetRef(0)
	goSrcStr := heap.GoString(jSrcStr)
	jTargetStr := heap.JString(frame.Method().Class().Loader(), goSrcStr+".so")
	frame.OperandStack().PushRef(jTargetStr)
}
...
func _sysProps() map[string]string {
	return map[string]string{
		...
		"java.library.path":    "/Library/Java/JavaVirtualMachines/jdk1.8.0_291.jdk/Contents/Home/jre/lib",
	}
}
